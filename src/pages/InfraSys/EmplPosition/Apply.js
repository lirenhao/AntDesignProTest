...
@connect(({ infra, type: sysType, }) => ({
  list: infra.list.emplPositionResponsibility || [],
...
class Apply extends PureComponent {
  ...
  saveRow(e, key) {
    ...
    if (!target.emplPositionId || !target.responsibilityTypeId ||
      !target.fromDate || !target.thruDate || !target.comments) {
  ...
  updateRow(e, key) {
    ...
    delete target.editable
    const { dispatch, emplPositionId } = this.props;
    target.emplPositionId = emplPositionId;
    dispatch({
      type: 'infra/update',
      payload: {
        type: 'emplPositionResponsibility',
