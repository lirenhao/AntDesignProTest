...
@connect(({ party, infra }) => ({
  list: party.list.emplPositionFulfillment || [],
  emplPositionList: infra.list.emplPosition,
  relationshipList: party.list.partyRelationship,
  personList: party.list.partyPerson || [],
}))
class Fulfillment extends PureComponent {
  index = 0;
  ...
  editRow = (e, key) => {
    ...
    const { data } = this.state;
    const target = list.filter(item => `${item.emplPositionId}-${item.partyId}`=== key)[0] || {};
  ...
  saveRow(e, key) {
    ...
    if (!target.partyId || !target.fromDate || !target.thruDate || !target.comments) {
    ...
    target.key = `${target.emplPositionId}-${target.partyId}`;
    dispatch({
  ...
  render() {
    const columns = [
      ...
      },
      {
        ...
        key: 'partyId',
        render: (text, record) => {
          const { relationshipList, personList } = this.props
          const lsit = relationshipList.map(r => personList.filter(p => p.partyId === r.partyIdTo)[0] || {})
          if (record.editable) {
            return (
              <Select 
                value={text}
                onChange={value => this.handleSelectFieldChange(value, 'partyId', record.key)}
                placeholder="当事人"
          ...
          const data = personList.filter(item => item.partyId === text)[0] || {}
    ...
    const dataSource = list.map(item => ({
      ...item,
      ...data[`${item.emplPositionId}-${item.partyId}`],
    }))
    ...
    return (
      <Fragment>
        <Table
          ...
          pagination={false}
          rowKey={record => `${record.emplPositionId}-${record.partyId}-${record.key}`}
        />
        <Button
