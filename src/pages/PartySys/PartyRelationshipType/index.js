...
const type = 'partyRelationshipType'
const id = 'partyRelationshipTypeId'
const pId = 'parentTypeId'
const title = 'partyRelationshipName'
const header = '当事人关系类型'

@connect(({ type: sysType, loading }) => ({
  sysType,
  tree: sysType.tree[type] || [],
  roleTypeTree: sysType.tree.roleType || [{children: []}],
          ...
          <Layout.Content>
            <Card title={`${header}编辑`}>
              <Form 
                info={info}
                tree={tree}
                roleTypeTree={roleTypeTree[0].children}
